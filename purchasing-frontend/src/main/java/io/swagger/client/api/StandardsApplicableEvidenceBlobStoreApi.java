...
public class StandardsApplicableEvidenceBlobStoreApi {
     ...
     * @throws RestClientException if an error occurs while attempting to invoke the API
     */
    public PaginatedListClaimBlobInfoMap apiStandardsApplicableEvidenceBlobStoreEnumerateClaimFolderTreeBySolutionIdGet(String solutionId, Integer pageIndex, Integer pageSize) throws RestClientException {
        Object postBody = null;
        
        // verify the required parameter 'solutionId' is set
        if (solutionId == null) {
            throw new HttpClientErrorException(HttpStatus.BAD_REQUEST, "Missing the required parameter 'solutionId' when calling apiStandardsApplicableEvidenceBlobStoreEnumerateClaimFolderTreeBySolutionIdGet");
        }
        
        // create path and map variables
        final Map<String, Object> uriVariables = new HashMap<String, Object>();
        uriVariables.put("solutionId", solutionId);
        String path = UriComponentsBuilder.fromPath("/api/StandardsApplicableEvidenceBlobStore/EnumerateClaimFolderTree/{solutionId}").buildAndExpand(uriVariables).toUriString();
