		...
		if (procurementId != 0) {
			...
			if (optProcurement.isPresent()) {
				...
				if (procurement.getSearchKeyword() != null && procurement.getSearchKeyword().trim().length() > 0) {
					...
					searchModel.setSolutions(onboardingService.findSolutionsHavingKeywords(searchModel.getSearchKeywords()));
					try {
						procurement = procurementService.saveCurrentPosition(procurement.getId(), secInfo.getOrgContactId(), Optional.of(searchModel.getSearchKeywords()), Optional.of(""), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty());
					} catch (Exception e) {
		...
		if (optProcurementId.isPresent()) {
			...
			try {
				...
				if (optProcurement.isPresent()) {
					...
					procurement = procurementService.saveCurrentPosition(optProcurementId.get(), secInfo.getOrgContactId(), Optional.of(searchModel.getSearchKeywords()), Optional.of(""), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty());
					...
					procurement = procurementService.saveCurrentPosition(0, secInfo.getOrgContactId(), Optional.of(searchModel.getSearchKeywords()), Optional.of(""), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty());
				}
				searchModel.setProcurement(procurement);
